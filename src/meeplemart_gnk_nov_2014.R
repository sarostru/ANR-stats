...
player_df <- build_player_data_frame("data/MM-141128/NRTM.json")
...
#Three at 2 points
identity_df['Quetzal',]$Average.Prestige <- 1.70
identity_df['Andromeda',]$Average.Prestige <- 2.00
identity_df['Iain Stirling',]$Average.Prestige <- 2.30
#Three at 4 points
identity_df['Whizzard',]$Average.Prestige <- 4.30
identity_df['The Foundry',]$Average.Prestige <- 4.00
identity_df['Exile',]$Average.Prestige <- 3.70
#Both at 5.5 points
identity_df['Leela Patel',]$Average.Prestige <- 5.65
identity_df['Nisei Division',]$Average.Prestige <- 5.35
#Both at 6 points
...
png("plots/MM-141128/identities_vs_presitge_all_players.png", width=24, height=10, units="in", res=72)
p2 <- ggplot(identity_df, aes(as.numeric(Number.of.Players), Average.Prestige, label=rownames(identity_df)))
p2 <- p2 + scale_x_continuous(breaks=seq(1, 8, 1)) + scale_y_continuous(breaks=seq(0, 8, 1))
...
p2 <- p2 + labs(title = "MeepleMart Toronto GNK Nov 2014\nNov 28 2014, 29 Players, 5 Rounds with Cards up to All That Remains", 
