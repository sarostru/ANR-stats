...
player_df <- build_player_data_frame("data/401-141208/NRTM.json")
...
#Both at 8 points
identity_df['Rielle "Kit" Peddler',]$Average.Prestige <- 8.15
identity_df['Building a Better World',]$Average.Prestige <- 7.85
#identity_df['Iain Stirling',]$Average.Prestige <- 2.30
#Three at 2 points
identity_df['Cerebral Imaging',]$Average.Prestige <- 1.70
identity_df['Ken "Express" Tenma',]$Average.Prestige <- 2.00
identity_df['Making News',]$Average.Prestige <- 2.30
#Both at 5 points
identity_df['Gabriel Santiago',]$Average.Prestige <- 5.15
identity_df['Personal Evolution',]$Average.Prestige <- 4.85
#Both at 6 points
...
png("plots/401-141208/identities_vs_presitge_all_players.png", width=24, height=10, units="in", res=72)
p2 <- ggplot(identity_df, aes(as.numeric(Number.of.Players), Average.Prestige, label=rownames(identity_df)))
p2 <- p2 + scale_x_continuous(breaks=seq(1, 5, 1)) + scale_y_continuous(breaks=seq(0, 8, 1))
...
p2 <- p2 + labs(title = "Toronto 401 Games O&C Box Dec 2014\nDec 8 2014, 26 Players, 5 Rounds with Cards up to All That Remains", 
...
swiss_rounds <- 5
#Rounding up n to an even number (i.e. it is 29, needs to be 30)
n <- n + (n%%2)
...
print("  Any missing games are byes or drops.")
